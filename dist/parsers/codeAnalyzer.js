...
function analyzeFileStructure(filePath) {
    ...
    function addEntity(type, name, node) {
        ...
        const key = name.trim();
        if (key) {
            if (!nameToEntityIds.has(key))
                nameToEntityIds.set(key, []);
    ...
    function statementChildren(node) {
    ...
    function firstMeaningfulStatement(node) {
        if (!node)
            return null;
    ...
    function connectSequentialStatements(ownerId, stmts, stmtToEntity) {
        for (let i = 0; i < stmts.length - 1; i++) {
            const a = stmtToEntity.get(stmts[i]);
            const b = stmtToEntity.get(stmts[i + 1]);
            if (a && b)
                addConnection(a, b, "next");
        }
        if (stmts.length > 0) {
            const first = stmtToEntity.get(stmts[0]);
            if (first)
                addConnection(ownerId, first, "entry");
        }
    }
    function buildControlFlowForBlock(owner, block) {
        const stmts = statementChildren(block);
        const stmtToEntity = new Map();
        for (const stmt of stmts) {
            ...
            if (stmt.type === "if_statement" || stmt.type === "switch_statement") {
                ...
                const consequence = stmt.childForFieldName("consequence") || stmt.namedChildren[0];
                ...
                if (trueBranch) {
                    const trueId = addEntity("condition", compactText("true: " + trueBranch.text), trueBranch);
                    addConnection(condId, trueId, "true");
                }
                if (falseBranch) {
                    const falseId = addEntity("condition", compactText("false: " + falseBranch.text), falseBranch);
                    addConnection(condId, falseId, "false");
                }
                continue;
            }
            if (stmt.type === "return_statement") {
                const returnExpr = stmt.namedChildren.find((c) => c.type.includes("expression"))?.text || "";
                const returnId = addEntity("return", compactText(returnExpr ? `return ${returnExpr}` : "return"), stmt);
            ...
            if (stmt.type === "variable_declaration" ||
                ...
                const vars = stmt.namedChildren.filter((c) => c.type === "variable_declarator");
                if (vars.length === 0) {
                    ...
                    const names = vars
                        ...
                        .filter(Boolean);
                    const varId = addEntity("variable", compactText(`declare ${names.join(", ")}`), stmt);
                    stmtToEntity.set(stmt, varId);
                    addConnection(owner.id, varId, "contains");
                }
                continue;
            ...
            if (calls.length > 0) {
                ...
                for (const targetId of targets) {
                    if (targetId !== callId)
                        addConnection(callId, targetId, "calls");
                }
            }
            else {
    ...
    function walk(node, scopeStack) {
        ...
        if (node.type === "function_declaration" ||
            ...
            for (const child of node.namedChildren) {
                if (child.type !== "statement_block")
                    walk(child, [...scopeStack, { id: fnId, name: fnName }]);
            }
            return;
        }
        if (node.type === "method_definition") {
            ...
            for (const child of node.namedChildren) {
                if (child.type !== "statement_block")
                    walk(child, [...scopeStack, { id: methodId, name: mName }]);
            }
            return;
        }
        for (const child of node.namedChildren) {
...
function dedupeConnections(input) {
    const seen = new Set();
    const out = [];
